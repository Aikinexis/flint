 ...
 */
export function History({ history: propHistory, onHistoryChange: _onHistoryChange }: HistoryProps) {
  // Component state
  const [history, setHistory] = useState<HistoryItem[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [isLoading, setIsLoading] = useState(true);
  const [selectedItem, setSelectedItem] = useState<HistoryItem | null>(null);
  const [activeFilters, setActiveFilters] = useState<Set<'generate' | 'summarize' | 'rewrite'>>(
    new Set()
  );
  ...
  const getOperationIcon = (type: HistoryItem['type']) => {
    switch (type) {
      case 'generate':
        return (
          <svg width="16" height="16" viewBox="0 0 56 56" fill="currentColor" aria-hidden="true">
  ...
  const handleToggleLiked = async (itemId: string, event: React.MouseEvent) => {
    ...
    setHistory((prev) =>
      prev.map((item) => (item.id === itemId ? { ...item, liked: !item.liked } : item))
    );
